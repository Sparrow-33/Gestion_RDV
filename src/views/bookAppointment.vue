<template >
  ...
  <div class="container px-5 py-24 mx-auto flex">
    <div class="lg:w-1/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
      <h2 class="text-gray-900 text-lg mb-1 font-medium title-font">Waiting for you</h2>
       <form @submit.prevent="getAppointment">
      ...
      <div class="relative mb-4">
        <label for="message" class="leading-7 text-lg font-bold  text-primary-3">Subject description</label>
           ...
           <input type="submit" value="Get Appointment" class="btn text-white px-3 py-2 rounded  w-full font-semibold bg-blue-500 hover:cursor-pointer hover:bg-blue-600 "> 
</form>
    </div>
  </div>
</section>
  ...
  methods:{
   ...
   //  let token = localStorage.getItem("token");
     /* let id = localStorage.getItem('id');
      ...
      const data = await fetch("http://localhost/app/appointments/takeAppointment",
          {
             method:"POST",
            headers:{
